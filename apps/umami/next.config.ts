...
const cloudMode = env.CLOUD_MODE || '';
const cloudUrl = env.CLOUD_URL || '';
const collectApiEndpoint = env.COLLECT_API_ENDPOINT || '';
const corsMaxAge = env.CORS_MAX_AGE || '';
const defaultLocale = env.DEFAULT_LOCALE || '';
const forceSSL = env.FORCE_SSL || '';
const frameAncestors = env.ALLOWED_FRAME_URLS || '';
const trackerScriptName = env.TRACKER_SCRIPT_NAME || '';
const trackerScriptURL = env.TRACKER_SCRIPT_URL || '';
